...
function StatCard({
  ...
  return (
    ...
    >
      {/* Icon */}
      <div
        className={`inline-flex items-center justify-center w-10 h-10 rounded-xl ${colors.bg} mb-3 ring-1 ${colors.ring} transition-all duration-300 group-hover:scale-110 group-hover:ring-2`}
      >
        <span className={`w-5 h-5 ${colors.text}`}>{icon}</span>
      </div>

      {/* Value */}
      <p className="text-2xl sm:text-3xl font-bold text-gray-900 counter-value tabular-nums tracking-tight">
        {formatNumber(count, locale)}
        <span className="text-base sm:text-lg font-semibold text-gray-400 ml-0.5">{suffix}</span>
      </p>
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-4">
        ...
        <p className="text-xs text-gray-400 tabular-nums">{formatNumber(total, locale)}</p>
      </div>
      ...
      <div className="relative h-2.5 bg-gray-100 rounded-full overflow-hidden">
        <motion.div
          initial={{ width: 0 }}
          animate={inView ? { width: `${malePct}%` } : {}}
          transition={{ duration: 1, delay: 0.6, ease: "easeOut" }}
          className="absolute inset-y-0 left-0 rounded-full bg-school-blue-500"
        />
        <motion.div
          initial={{ width: 0 }}
          animate={inView ? { width: `${femalePct}%` } : {}}
      ...
      <div className="flex justify-between mt-3">
        <div className="flex items-center gap-2">
          <span className="w-2.5 h-2.5 rounded-full bg-school-blue-500" />
          <span className="text-xs sm:text-sm text-gray-600">
            {locale === "km" ? "ប្រុស" : "Male"}
          </span>
          <span className="text-xs sm:text-sm font-semibold text-gray-900 tabular-nums">
            {formatNumber(male, locale)}
            <span className="text-gray-400 font-normal ml-1">({malePct.toFixed(1)}%)</span>
          </span>
        </div>
        <div className="flex items-center gap-2">
          <span className="w-2.5 h-2.5 rounded-full bg-amber-400" />
          <span className="text-xs sm:text-sm text-gray-600">
            {locale === "km" ? "ស្រី" : "Female"}
          </span>
          <span className="text-xs sm:text-sm font-semibold text-gray-900 tabular-nums">
            {formatNumber(female, locale)}
            <span className="text-gray-400 font-normal ml-1">({femalePct.toFixed(1)}%)</span>
          </span>
        </div>
      </div>
    </motion.div>
    ...
    <section className="py-16 sm:py-20 bg-gray-50/80">
      <div className="container mx-auto px-4">
        ...
        <div className="mt-6 sm:mt-8 max-w-2xl mx-auto">
          <GenderCard
            male={stats.male_students ?? 0}
            female={stats.female_students ?? 0}
